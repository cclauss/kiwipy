...
__all__ = ['TornadoConnection', 'TornadoChannel']
...
class TornadoConnection(object):
    def __init__(self, parameters, ioloop=None):
        if ioloop is None:
            ioloop = tornado.ioloop.IOLoop.current()

        self._open_future = Future()

        # This future will be set if the user initiates an orderly connection close
        self._close_future = None

        # These futures are used while a channel is being opened
        self._channel_open_futures = []

        self._impl = pika.TornadoConnection(
    ...
    def add_on_connection_blocked_callback(self, callback):
        self._impl.add_on_connection_blocked_callback(_CallbackWrapper(self, callback))

    def add_on_connection_unblocked_callback(self, callback):
        self._impl.add_on_connection_unblocked_callback(_CallbackWrapper(self, callback))
    ...
    def add_timeout(self, deadline, callback):
        ...
        """
        return self._impl.add_timeout(deadline, callback)
    ...
    def add_callback_threadsafe(self, callback):
        ...
        """
        self._impl.add_callback_threadsafe(callback)
    ...
    def channel(self, channel_number=None):
        ...
        try:
            self._channel_open_futures.append(open_future)
            result = yield open_future
        finally:
            self._channel_open_futures.remove(open_future)

        # Create our proxy channel
        channel = TornadoChannel(impl_channel, self)
    ...
    def ensure_connected(self):
        yield self._open_future

    def _on_open(self, connection):
        assert connection is self._impl
        self._open_future.set_result(True)

    def _on_open_error(self, connection, exception_or_message):
        assert connection is self._impl
        if isinstance(exception_or_message, Exception):
            self._open_future.set_exception(exception_or_message)
        else:
            # We got a string message
            self._open_future.set_exception(pika.exceptions.AMQPConnectionError(exception_or_message))
    ...
    def _on_close(self, connection, reply_code, reply_text):
        ...
        if self._channel_open_futures:
            # Set exceptions on all channel open attempts
            exception = pika.exceptions.ConnectionClosed(reply_code, reply_text)
            for future in self._channel_open_futures:
                future.set_exception(exception)


DeliveryInfo = namedtuple('DeliveryInfo', ['tag', 'future'])

ReturnedMessage = namedtuple('ReturnedMessage', ['method', 'properties', 'body'])


class TornadoChannel(object):
    ...
    def __init__(self, channel_impl, connection):
        ...
        :param channel_impl: The pika channel object
        :type channel_impl: :class:`pika.Channel`
    ...
    def _on_channel_closed(self, _channel, reply_code, reply_text):
        ...
        # Figure out if the whole connection went down or just the channel
    ...
    def add_on_close_callback(self, callback):
        ...
        """
        self._impl.add_on_close_callback(_CallbackWrapper(self, callback))
    ...
    def add_on_return_callback(self, callback):
        ...
        """
        self._impl.add_on_return_callback(_CallbackWrapper(self, callback))
    ...
    def basic_consume(self,
        ...
        with self._pending_reply(pika.spec.Basic.ConsumeOk) as ok_result:
            tag = self._impl.basic_consume(
                on_message_callback=_CallbackWrapper(self, on_message_callback),
                queue=queue,
                auto_ack=auto_ack,
                exclusive=exclusive,
